...
export function CategoryBox ({notes, category, goToCategory}) {
	return (
		<div className={category.id}>
			<div className="bg-white text-800 rounded shadow cursor-pointer hover:ring hover:ring-500" onClick={() => goToCategory(category.id)}>
				<div className="py-8 bg-200 flex">
					<h1 className="bg-white px-6 py-2 m-auto rounded">{notes.filter(n => n.categoryId === category.id).length}</h1>
				</div>
